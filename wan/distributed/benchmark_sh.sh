...
# 设置GPU时钟频率
yes y | sudo rocm-smi --setsrange 500 600
mkdir -p /home/jialzhu/clk_500_600

# 启动后台监控进程，每0.1秒记录一次
LOG_FILE="/home/jialzhu/clk_500_600/amd_smi_monitor_$(date +%Y%m%d_%H%M%S)_500_600.log"
echo "开始监控，当前时钟频率 500-600，日志文件：$LOG_FILE"
...
# 在容器内压缩JSON文件
ARCHIVE_NAME="benchmark_results_$(date +%Y%m%d_%H%M%S)_500_600.7z"
echo "正在压缩结果文件..."
docker exec zov_wan2.2_rope_alltoall_fusion_test bash -c "cd /app/Wan2.2/wan/distributed && 7z a -mx=9 $ARCHIVE_NAME *.json"

# 复制压缩包到宿主机
docker cp zov_wan2.2_rope_alltoall_fusion_test:/app/Wan2.2/wan/distributed/$ARCHIVE_NAME /home/jialzhu/clk_500_600/
echo "压缩包已复制到 /home/jialzhu/clk_500_600/$ARCHIVE_NAME"
